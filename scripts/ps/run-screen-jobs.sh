...
#   psd|art.png|out.png|so indices|solo group|trim margin|keep background|keep gloss|extra layers to hide
...
while IFS='|' read -r psd art out idx solo margin bg gloss hide; do
  [ -z "$psd" ] && continue
  case "$psd" in \#*) continue;; esac
  printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$psd" "$art" "$out" "$idx" "${solo:--1}" "${margin:--1}" "${bg:-1}" "${gloss:-1}" "${hide:--}" \
